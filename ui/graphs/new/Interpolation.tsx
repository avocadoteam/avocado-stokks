import type { AnimatedProps, PathProps, SkPath } from '@shopify/react-native-skia';
import {
  Canvas,
  Easing,
  Fill,
...
export const Interpolation: React.FC<GraphProps> = ({ height, width }) => {
  const [currentPath, setCurrentPath] = useState(() => createGraphPath(width, height, 60));
...
const TransitioningPath = ({ path, ...props }: AnimatedProps<PathProps> & { path: SkPath }) => {
  ...
  const animatedPath = useDerivedValue(
    () => interpolatePaths(progress.current, [0, 1], [currentPathRef.current, nextPathRef.current]),
    [progress, path],
  );
